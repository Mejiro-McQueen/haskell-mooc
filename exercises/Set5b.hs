...
cull _ Empty = Empty
cull val (Node a l r) 
  | a /= val = Node a (cull val l) (cull val r)
  | otherwise = Empty 
...
walk [StepL] (Node a l r) = walk [] l
walk [StepR] (Node a l r) = walk [] r
walk (StepR:rest) (Node a l r) = walk rest r
walk (StepL:rest) (Node a l r) = walk rest l
...
search n (Node a l r) 
  | n /= a = if null result then Nothing else Just result
  | otherwise  = Just []
  where
    result = travel [StepL] l ++ travel [StepR] r
    travel path Empty = [] 
    travel path (Node a l r) 
      | n == a = path
      | otherwise = travel (path++[StepL]) l ++ travel (path++[StepR]) r