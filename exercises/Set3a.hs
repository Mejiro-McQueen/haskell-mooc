...
firstHalf s = if (odd . length) s then take (n+1) s else take n s
    where
        n = div (length s) 2
...
capitalize = unwords . map capfirst . words
    where capfirst a = (toUpper . head) a : tail a
...
-- This excercise is not correct 
powers :: Int -> Int -> [Int]
powers k max = takeWhile  (\x-> k^x <= max) [1..]
...
whileRight f a = either id (whileRight f ) (f a)
...
joinToLength n s = filter (\x->length x == n) [a++b | a <-s, b <-s]
...
multiCompose fs x = foldr ($) x fs  -- wow
...
interpreter [] = [""]
interpreter cs = interpret (0,0) cs 
    where 
        interpret (x,y) ("right":rest) = interpret (x+1,y) rest 
        interpret (x,y) ("left":rest) = interpret (x-1,y) rest 
        interpret (x,y) ("up":rest) = interpret (x,y+1) rest 
        interpret (x,y) ("down":rest) = interpret (x,y-1) rest
        interpret (x,y) ("printX":rest) = show x: interpret (x,y) rest 
        interpret (x,y) ("printY":rest) = show y: interpret (x,y) rest 
        interpret _ _ = [] 

-- This was easy the first time...