...
import GHC.Base (Semigroup, Alternative (empty))
import Network.HTTP (URIAuthority(password))
import Network.HTTP.Base (URIAuthority(password))
...
add a set@(Set s) = if member a set then set else Set(sort (a:s))
...
data State = Start | Error | Finished | Egged | Floured | Mixable | Bakeable | Sugared
  deriving (Eq,Show)
...
average (x:|[]) = x
average allx@(x:|xs) = sum allx / fromIntegral (length allx)
...
reverseNonEmpty (a:|[]) = a:|[]
reverseNonEmpty (a:|b) = last :| (tail reversed++[a])
  where
    reversed = reverse b
    last = head reversed
    rest = tail reversed
...
show2 :: (Operation2 op, Show op) => op -> String
show2 = show
...
passwordAllowed p (ContainsSome []) = False
passwordAllowed p (ContainsSome (l:ls)) = elem l p  || passwordAllowed p (ContainsSome ls)
passwordAllowed p (DoesNotContain []) = True
passwordAllowed p (DoesNotContain (l:ls)) = notElem l p && passwordAllowed p (DoesNotContain ls)
...
data Arithmetic = Literal Integer | Multiply Arithmetic Arithmetic | Add Arithmetic Arithmetic  
  deriving Show
