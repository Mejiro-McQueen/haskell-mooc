...
mapMaybe a (Just b) = Just (a b)
mapMaybe a Nothing = Nothing
...
firstHalf s
    | (odd . length) s = take (mid + 1) s
    | otherwise = take mid s
    where mid = div (length s) 2
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) powers
    where powers = map (k^) [0..max]
...
whileRight check x = continue (check x)
    where
        continue (Left a)  = a
        continue (Right f) = whileRight check f
...
joinToLength n xs = [x ++ y | x <- xs, y <- xs, length x + length y==n]
