...
oddFactorial a
    | a == 0 = 1
    | even a = 1 * oddFactorial (a-1)
    | odd a = a * oddFactorial (a-1)
    | otherwise = 1
...
countdown n = "Ready! " ++ (nums n) ++ " Liftoff!"
    where 
        nums n
            | n == 1 = show n ++ "... "
            | otherwise  = show n ++ "... " ++ nums (n-1)
...
smallestDivisor n = something n 2
    where something n b
            | mod n b == 0 = b
            | otherwise = something n (b+1)
...
biggestPrimeAtMost n = something n n
    where 
        something a b 
            | isPrime b = b
            | otherwise = something a (b-1)
