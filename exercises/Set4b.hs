...
countHelper x val = maybe (val+1) (const val) x
...
concatHelper :: [a] -> [a] -> [a] 
concatHelper next current = next ++ current
...
largestHelper val occurences 
    | [val] > occurences = [val]
    | val == head occurences = occurences++[val]
    | otherwise = occurences
...
headHelper x (Just current) = Just x
...
lastHelper x (Just current) = Just current
