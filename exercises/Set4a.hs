...
allEqual [] = True
allEqual (x:y) = res x y
    where
        res x (y:xs) = x == y && res y xs
        res x [] = True
...
longest (x:y:xs)
        ...
        | otherwise  = if head x < head y then longest (x:xs) else longest (y:xs)
...
incrementKey _ [] = []
incrementKey target ((k,v):rest)
    | k == target = (k,v+1):incrementKey target rest
    | otherwise = (k,v):incrementKey target rest
...
freqs = foldr (Map.alter plus1) Map.empty
    where
        plus1 (Just v) = Just (v+1)
...
transfer from to amount bank
    ...
    | Map.findWithDefault 0 from bank < amount || amount <= 0 = bank
    | otherwise = (adjustTo . adjustFrom) bank
    where
        adjustTo = Map.adjust (+amount) to
        adjustFrom = Map.adjust (+(-amount)) from
------------------------------------------------------------------------------
...
maxIndex arr =  maximumBy  t (assocs arr)
    where t = compare `on` (maximum) (snd)
