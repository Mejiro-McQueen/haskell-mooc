...
takeFinal n xs
    | listlength < n = xs
    | otherwise = drop (listlength - n) xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = front ++ [x] ++ rear
    where
        front
            | i == 0 = []
            | otherwise = take i xs
        rear = takeFinal (length xs - i -1) xs
...
palindromify s 
    | isPalindrome s = s 
    | otherwise = palindromify chopped_string
    where chopped_string = substring 1 (length s-1) s
...
greet first last = "Hello, " ++ naming first last ++ "!"
    where 
        naming first Nothing = first
        naming first (Just last) = first ++ " " ++ last
...
addEithers a b = something a b 
    where 
        something (Right a) (Right b) = Right $ a + b
        something (Right a) fail@(Left b) = fail
        something fail@(Left a) _ = fail