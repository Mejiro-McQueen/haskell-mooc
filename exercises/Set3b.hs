...
sums :: Int -> [Int]
sums 0 = [0]
sums i = traverse 0 1 i
    where
        traverse :: Int -> Int -> Int -> [Int]
        traverse prev counter stop
            | counter == stop = [prev + i]
            | otherwise = (prev + counter):traverse (prev + counter) (counter + 1) stop
...
sorted :: [Int] -> Bool
sorted [] = True
sorted (x:xs) = ord x xs
    where
        ord prev (curr:rest) = prev <= curr && ord curr rest
        ord _ [] = True
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf (x:xs) = recurse x xs
    where
        recurse prev (curr:rest) = prev:recurse (prev+curr) rest
        recurse prev [] = [prev]
...
merge :: [Int] -> [Int] -> [Int]
merge xs ys = recurse xs ys
    where
        recurse (x:xs) (y:ys)
            | x <= y = x:recurse xs (y:ys)
            | otherwise = y:recurse (x:xs) ys
        recurse [] [] = []
        recurse xs [] = xs
        recurse [] ys = ys
...
maybeMap _ [] = [] 
maybeMap f (x:xs) = res (f x)
    where 
        res (Just x) = x:maybeMap f xs 
        res Nothing = maybeMap f xs
